...
class OlmoeAttention(nn.Module):
    ...
    def forward(
        ...
        set_patch = None, #### added, for replacing tensors
        **kwargs,
        ...
        if set_patch is not None:
            for p in set_patch:
                match p[0]:
                    ...
                    case 'v':
                        value_states = p[2]
                    case 'q_head': # p[2]: patched_head; p[3]: patched_token_ls
                        query_states[torch.arange(bsz), p[2], p[3]] = p[4]
                    case 'k_head':
                        key_states[torch.arange(bsz), p[2], p[3]] = p[4]
                    case 'v_head':
                        value_states[torch.arange(bsz), p[2], p[3]] = p[4]
        ########
        ...
        hook_q2, hook_k2 = query_states.detach().clone(), key_states.detach().clone() #### unused, added, after pos embeddings. NOTE: q2 and k2 should be used for patching one head, one token

        attn_weights = torch.matmul(query_states, key_states.transpose(2, 3)) / math.sqrt(self.head_dim)
        #### added, checker
        # tmp = attn_weights[0, 3, 4, 5]
        # tmp2 = torch.dot(query_states[0, 3, 4], key_states[0, 3, 5]) / math.sqrt(self.head_dim)
        # print("check attn_weights", torch.isclose(tmp, tmp2))
        ########
        #### added
        if set_patch is not None:
            for p in set_patch:
                match p:
                    case 'q2t': # only influence single token; p[2]: patched_head; p[3]: patched_q_token_ls; p[4]: patched_k_token_ls; p[5]: specified q2
                        attn_weights[torch.arange(bsz), p[2], p[3], p[4]] = torch.dot(p[5], key_states[torch.arange(bsz), p[2], p[3]]) / math.sqrt(self.head_dim)
                    case 'k2t': # only influence single token; p[2]: patched_head; p[3]: patched_q_token_ls; p[4]: patched_k_token_ls; p[5]: specified k2
                        attn_weights[torch.arange(bsz), p[2], p[3], p[4]] = torch.dot(p[5], query_states[torch.arange(bsz), p[2], p[3]]) / math.sqrt(self.head_dim)
        ########
        ...
        hook_attn_weights_before_softmax = attn_weights.detach().clone()
        attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query_states.dtype)
        attn_weights = nn.functional.dropout(attn_weights, p=self.attention_dropout, training=self.training)
        attn_output = torch.matmul(attn_weights, value_states)
        #### added, checker
        # tmp = attn_output[0, 3, 4]
        # tmp2 = torch.zeros_like(tmp)
        # tmp2 = torch.sum([attn_weights[0, 3, 4, i] * value_states[0, 3, i] for i in range(q_len)])
        # print("check attn_output", torch.isclose(tmp, tmp2))
        ########
        #### added
        hook_attn_weights = attn_weights #### added, shape: [batch_size, num_heads, num_q, num_k]
        hook_before_matmul_wo = attn_output #### added, shape: [batch_size, num_heads, num_tokens, head_dim]
        if set_patch is not None:
            for p in set_patch:
                if p[0] == "before_matmul_wo":
                    attn_output = p[3]
                # elif p[0] == "before_matmul_wo_head_token": # p[2]: patched_head; p[3]: patched_token_ls
                #     attn_output[torch.arange(bsz), p[2], p[3]] = p[4]
                # elif p[0] == "v2t": # only influence single token; p[2]: patched_head, p[3]: patched_q_token_ls; p[4]: patched_k_token_ls; p[5]: specified v
                #     attn_weights_h_q_k = attn_weights[0, p[2], p[3], p[4]]
                #     attn_output[0, p[2], p[3]] -= (attn_weights_h_q_k * value_states[0, p[2], p[4]])
                #     attn_output[0, p[2], p[3]] += (attn_weights_h_q_k * p[5])
        ########
        ...
        attn_hooks = (hook_q, hook_k, hook_v, hook_q2, hook_k2, hook_attn_weights_before_softmax, hook_attn_weights, hook_before_matmul_wo) #### added
        return attn_output, attn_weights, past_key_value, attn_hooks #### added
        # return attn_output, attn_weights, past_key_value #### commented
...
class OlmoeSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, cur_layer_counter) -> torch.Tensor:
        ...
        routing_weights, selected_experts = torch.topk(routing_weights, self.top_k, dim=-1) #### FIXME: temporary commented for score gap experiment
        #### FIXME: for score gap experiment only
        # routing_weights, selected_experts = torch.topk(routing_weights, 64, dim=-1)
        # routing_weights = torch.cat((routing_weights[:, 0:7], routing_weights[:, 8:9]), dim=1)
        # selected_experts = torch.cat((selected_experts[:, 0:7], selected_experts[:, 8:9]), dim=1)
        ####
        ...
        for expert_idx in range(self.num_experts):
            ...
            #### FIXME: for temporary experiment only (knockout M1E9/M4E14)
            # if cur_layer_counter == 1 and expert_idx == 9:
            #     routing_weights[top_x, idx, None] = 0
            ########
...
class OlmoeDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        cur_layer_counter = 0, #### added, for getting the current layer id
        **kwargs,
        ...
        hidden_states, router_logits, mlp_hooks = self.mlp(hidden_states, cur_layer_counter) #### modified
...
class OlmoeModel(OlmoePreTrainedModel):
    ...
    def forward(
        ...
        hook_k = torch.empty((n_layers, batch_size, n_heads, n_tokens, n_head_dim))
        hook_v = torch.empty((n_layers, batch_size, n_heads, n_tokens, n_head_dim))
        hook_q2 = torch.empty((n_layers, batch_size, n_heads, n_tokens, n_head_dim))
        hook_k2 = torch.empty((n_layers, batch_size, n_heads, n_tokens, n_head_dim))
        hook_attn_weights_before_softmax = torch.empty((n_layers, batch_size, n_heads, n_tokens, n_tokens))
        hook_attn_weights = torch.empty((n_layers, batch_size, n_heads, n_tokens, n_tokens))
        hook_before_matmul_wo = torch.empty((n_layers, batch_size, n_heads, n_tokens, n_head_dim))
        ...
        for decoder_layer in self.layers[: self.config.num_hidden_layers]:
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    ...
                    position_embeddings=position_embeddings,
                    set_patch=set_patch, 
                    cur_layer_counter=layer_counter#### added
                )
            ...
            #### added
            hook_layer_input[layer_counter] = layer_outputs[dict_pos][0]
            hook_attn_output[layer_counter] = layer_outputs[dict_pos][1]
            hook_after_res1[layer_counter] = layer_outputs[dict_pos][2]
            hook_after_norm2[layer_counter] = layer_outputs[dict_pos][3]
            hook_mlp_output[layer_counter] = layer_outputs[dict_pos][4]
            hook_layer_output[layer_counter] = layer_outputs[dict_pos][5]
            hook_q[layer_counter] = layer_outputs[dict_pos][6]
            hook_k[layer_counter] = layer_outputs[dict_pos][7]
            hook_v[layer_counter] = layer_outputs[dict_pos][8]
            hook_q2[layer_counter] = layer_outputs[dict_pos][9]
            hook_k2[layer_counter] = layer_outputs[dict_pos][10]
            hook_attn_weights_before_softmax[layer_counter] = layer_outputs[dict_pos][11]
            hook_attn_weights[layer_counter] = layer_outputs[dict_pos][12]
            hook_before_matmul_wo[layer_counter] = layer_outputs[dict_pos][13]
            router_logits[layer_counter] = layer_outputs[dict_pos][14]
        ...
        hook_q2 = hook_q2.transpose(0,1)
        hook_k2 = hook_k2.transpose(0,1)
        hook_attn_weights_before_softmax = hook_attn_weights_before_softmax.transpose(0,1)
        hook_attn_weights = hook_attn_weights.transpose(0,1)
        hook_before_matmul_wo = hook_before_matmul_wo.transpose(0,1)
        ...
        hook_dict = {"hook_layer_input" : hook_layer_input, "hook_attn_output" : hook_attn_output,
                    ...
                    "hook_q" : hook_q, "hook_k" : hook_k,
                    "hook_v" : hook_v, "hook_q2" : hook_q2, "hook_k2" : hook_k2,
                    "hook_attn_weights_before_softmax" : hook_attn_weights_before_softmax,
