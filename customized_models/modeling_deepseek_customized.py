...
class MoEGate(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if self.training and self.alpha > 0.0:
            ...
            aux_loss = None
        # return topk_idx, topk_weight, aux_loss #### commented
        return topk_idx, topk_weight, aux_loss, router_logits
...
class DeepseekV2MoE(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # topk_idx, topk_weight, aux_loss = self.gate(hidden_states) #### commented
        topk_idx, topk_weight, aux_loss, router_logits = self.gate(hidden_states) #### added
        ...
        if self.training:
            ...
            # y = self.moe_infer(hidden_states, topk_idx, topk_weight).view(*orig_shape) #### commented
    ...
    def moe_infer(self, x, topk_ids, topk_weight):
        ...
        new_x = torch.empty_like(outs)
        new_x[idxs] = outs
        hook_expert_weighted_outputs = new_x.view(*topk_ids.shape, -1).type(topk_weight.dtype).mul_(topk_weight.unsqueeze(dim=-1)) #### added
        final_out = (
        ...
        # return final_out #### commented
        return final_out, hook_expert_weighted_outputs
...
class DeepseekV2Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hook_q, hook_k, hook_v = query_states.detach().clone(), key_states.detach().clone(), value_states.detach().clone() # added
        attn_weights = (
        ...
        hook_attn_weights = attn_weights #### added, shape: [batch_size, num_heads, num_q, num_k] # NOTE: hook_attn_weights_before_softmax unimplemented.
        ...
        attn_hooks = (hook_q, hook_k, hook_v, None, None, hook_attn_weights, hook_before_matmul_wo) #### added
...
class DeepseekV2DecoderLayer(nn.Module):
    def __init__(self, config: DeepseekV2Config, layer_idx: int):
        ...
        self.has_moe = (True if layer_idx >= config.first_k_dense_replace else False) #### added
    ...
    def forward(
        ...
        **kwargs,
        ...
        # hidden_states = self.mlp(hidden_states) #### commented
        #### added
        if self.has_moe:
            hidden_states, mlp_hooks = self.mlp(hidden_states)
        else:
...
class DeepseekV2Model(DeepseekV2PreTrainedModel):
    ...
    def forward(
        ...
        n_qk_head_dim = self.config.qk_nope_head_dim + self.config.qk_rope_head_dim
        ...
        hook_q = torch.empty((n_layers, batch_size, n_heads, n_tokens, n_qk_head_dim)) # different from olmoe
        hook_k = torch.empty((n_layers, batch_size, n_kv_heads, n_tokens, n_qk_head_dim)) # different from olmoe
        hook_v = torch.empty((n_layers, batch_size, n_kv_heads, n_tokens, n_head_dim)) # different from olmoe
        # hook_q2 = torch.empty((n_layers, batch_size, n_kv_heads, n_tokens, n_head_dim)) # different from olmoe
        # hook_k2 = torch.empty((n_layers, batch_size, n_kv_heads, n_tokens, n_head_dim)) # different from olmoe
        ...
        for decoder_layer in self.layers:
            ...
            #### added
            # for i in range(len(layer_outputs[dict_pos])):
            #     if layer_outputs[dict_pos][i] is not None:
            #         print(i, layer_outputs[dict_pos][i].shape)
            ...
            hook_attn_weights[layer_counter] = layer_outputs[dict_pos][11]
            hook_before_matmul_wo[layer_counter] = layer_outputs[dict_pos][12]
            if layer_counter > 0:
                router_logits[layer_counter] = layer_outputs[dict_pos][13]
